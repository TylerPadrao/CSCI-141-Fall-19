...
def caesar(original, codeword):
    ...
    for idx in range(len(codeword)):
        c = ord(codeword[idx]) - ord(original[idx])
        if c < 0:
            c = c + 26
        if c != shift:
            return -1
...
def test_function1(original, codeword, expected):
    """
    ...
    and the actual result. If the test passed, it will print passed.
    """
    result = caesar(original, codeword)
    if result == expected:
        print("Passed")
    else:
...
def run_all_tests():
    """
    This is a test function that runs all of the tests, when the user inputs original and code word tests with an
    expected value, from the function "test_function1(original, codeword, expected)".
    """
    test_function1("VERY", "AJWD", 5)
    test_function1("DOG", "JUM", 6)
    test_function1("nope", "opqf", 1)
    test_function1("hey", "roi", 10)
    test_function1("yo", "cs", 4)
    test_function1("xyz", "abc", 3)
